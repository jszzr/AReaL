...
def do_deregister(
    ...
    with store.lock_model_state(service_name):
        ...
        try:
            ...
            if exc.status != 404:
                logger.warning(
                    "router remove_model %s returned %d", model_name, exc.status
                )
        ...
        for r in entry.replicas:
            if r.router_worker_id is None:
                logger.warning(
                    "data-proxy %s has no router_worker_id in legacy model state; "
                    "skipping Router unregister to avoid deleting a newer incarnation",
                    r.data_proxy.addr,
                )
                continue
            try:
                router.unregister_worker(r.data_proxy.addr, r.router_worker_id)
            except (ServiceHTTPError, ServiceUnreachable) as exc:
                logger.warning(
                    "router unregister %s failed: %s", r.data_proxy.addr, exc
                )
